...
export default function Page() {
  return (
    <>
      ...
      {/* No approved photography exists for the IT division (survey F-09), so this
          hero is typographic rather than borrowing an unrelated image. */}
      <section className="relative flex min-h-[52svh] items-end bg-[radial-gradient(120%_90%_at_78%_12%,var(--forest-800)_0%,var(--ink)_62%)] text-on-dark">
        <div className="container pb-[clamp(3rem,6vw,5rem)] pt-36">
          <p className="eyebrow text-champagne">02 — IT Services</p>
          <h1 className="display mt-6 max-w-[18ch] text-h1">Selected Project Experience</h1>
          <p className="mt-7 max-w-[62ch] text-lede leading-relaxed text-on-dark-muted">
            {itProjectsIntro}
          </p>
        </div>
      </section>
            ...
            <Reveal key={p.slug} delay={i * 60}>
              <article className="grid gap-8 lg:grid-cols-12 lg:gap-14">
                ...
                <div className="lg:col-span-4">
                  ...
                  <h2 className="display mt-4 text-h2">{p.name}</h2>
                  <a
                    href={p.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-6 inline-flex items-center gap-2 border-b border-line-light pb-1 text-[0.88rem] text-on-light-muted transition-colors hover:border-forest-600 hover:text-forest-600"
                  >
                    Visit live site
                  ...
                  <ol className="mt-8 flex flex-wrap items-center gap-x-3 gap-y-2">
                    {p.journey.map((step, si) => (
                      <li key={step} className="flex items-center gap-3">
                        <span className="eyebrow text-forest-600">{step}</span>
                ...
                <div className="border border-line-light bg-ivory-raised p-6">
                  <div className="flex flex-wrap items-baseline justify-between gap-4">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[0.85rem] text-on-light-muted underline underline-offset-4 hover:text-forest-600"
                    >
                      {b.url.replace("https://", "")}
                    </a>
                  </div>
